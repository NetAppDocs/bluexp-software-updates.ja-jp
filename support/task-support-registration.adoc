...
+
これはシステムによって生成されたIDで、電子メールにマッピングされます。NSS管理*ページでは、メニューからEメールを表示できます image:https://raw.githubusercontent.com/NetAppDocs/bluexp-family/main/media/icon-nss-menu.png["3つの水平ドットのアイコン"] 。

** ログイン資格情報トークンを更新する必要がある場合は、メニューに*資格情報の更新*オプションもあり image:https://raw.githubusercontent.com/NetAppDocs/bluexp-family/main/media/icon-nss-menu.png["3つの水平ドットのアイコン"] ます。
+
